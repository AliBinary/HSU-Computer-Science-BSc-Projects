...
def Draw():
    ...
    pygame.draw.rect(Surface, (0, 0, 0), (CenterX-(Width/2) -
                                          1, Space, Width+2, Height+2), 1)
    pygame.draw.rect(Surface, (r, g, b),
                     (CenterX-(Width/2), Space+1, Width, Height), 0)
    ...
    ren3 = Font.render("OpenGL: %s" %
                       RGBToOpenGLColor((r, g, b)), True, (0, 0, 0))
    pos3 = ((Screen[0]/2)-(ren3.get_width()/2), Height+(2*Space)+40)
    Surface.blit(ren3, pos3)

    if hasattr(pygame, "scrap"):
        rect1 = pygame.Rect(
            pos1[0], pos1[1], ren1.get_width(), ren1.get_height())
        rect2 = pygame.Rect(
            pos2[0], pos2[1], ren2.get_width(), ren2.get_height())
        rect3 = pygame.Rect(
            pos3[0], pos3[1], ren3.get_width(), ren3.get_height())
        Collision = False
        if rect1.collidepoint(mpos):
            Collision = True
            pygame.draw.rect(Surface, (0, 0, 255), rect1, 1)
            ColorText = (r, g, b)
        if rect2.collidepoint(mpos):
            Collision = True
            pygame.draw.rect(Surface, (0, 0, 255), rect2, 1)
            ColorText = RGBToHTMLColor((r, g, b))
        if rect3.collidepoint(mpos):
            Collision = True
            pygame.draw.rect(Surface, (0, 0, 255), rect3, 1)
            ColorText = RGBToOpenGLColor((r, g, b))
        if Collision:
