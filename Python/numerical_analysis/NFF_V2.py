...
def Try_values():
    ...
    for i in range(num_points):
        ...
        if child1.get() and child2.get():
            if not (child1.get().isnumeric() and child2.get().isnumeric()):
                notnum = True
                break
            flag += 1
        elif child1.get():
            if not (child1.get().isnumeric()):
                notnum = True
                break
            flag += 1
        elif child2.get():
            if not (child2.get().isnumeric()):
                notnum = True
                break
            flag += 1
...
def show_points():
    ...
    if not num_points_combobox.get():
...
num_points_combobox = ttk.Combobox(
    frm_Cambox, values=[2, 3, 4, 5, 6, 7, 8, 9, 10], width=10)
button = tk.Button(frm_Cambox, text="Show Points", command=show_points)
...
btn_remove.grid(row=0, column=0, sticky="s")
btn_add.grid(row=0, column=1, sticky="s")
